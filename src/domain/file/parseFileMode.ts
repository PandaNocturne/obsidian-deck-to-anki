...
import { buildTitleNode } from '../title/buildTitleTree';
 ...
 * - head / card / list / title �??use as-is
...
export async function parseFileMode(
	...
	for (const ref of refs) {
		...
		if (resolved.autoSetHead) {
			warnings.push(`${childName}: ?�?��?��??�??deckType: head / deckFile`);
		}
		...
		if (resolved.deckType === 'card' || resolved.deckType === 'title') {
			const leafTrail = [
				{
					name: deckName,
					sourceFilePath: filePath,
			...
			const parsedLeaf =
				resolved.deckType === 'title'
					? buildTitleNode({
							filePath: dest.path,
							content: childContent,
							deckName: childName,
							parentDeckPath: root.deckPath,
							deckBacklinkTrail: leafTrail,
						})
					: buildCardNode({
							filePath: dest.path,
							content: childContent,
			...
			warnings.push(
				...parsedLeaf.warnings.map((w) => `${childName}: ${w}`),
			);
			if (parsedLeaf.card) {
				// Card/title notes are leaves under the file deck — no nested deck row.
				root.children.push(parsedLeaf.card);
			}
			continue;
		}
